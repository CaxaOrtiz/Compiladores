 ...
 """
    ...
    def for_stmt_3(self):
        ...
        if self.currentToken.get_tipo() in [TipoToken.THIS, TipoToken.SUPER, TipoToken.FALSE, TipoToken.TRUE, TipoToken.NULL,
            ...
            # Si el tipo del token actual no coincide con los tipos de token esperados, se asume épsilon, lo que significa que no hay expresión en esta parte del bucle FOR

            def if_stmt(self):
                # Coincide con el token IF
                self.match(TipoToken.IF)
                # Coincide con el token PARENTESIS_IZQUIERDO
                self.match(TipoToken.PARENTESIS_IZQUIERDO)
                # Analiza la expresión que sigue al IF llamando al método expression()
                self.expression()
                # Coincide con el token PARENTESIS_DERECHO
                self.match(TipoToken.PARENTESIS_DERECHO)
                # Analiza la sentencia dentro del IF llamando al método statement()
                self.statement()
                # Analiza la parte ELSE del IF llamando al método else_statement()
                self.else_statement()
